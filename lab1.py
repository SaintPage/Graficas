...
class ScanlineRenderer(Renderer):
    def glFillPolygon(self, poly, color=None):
        # 1) Fijar el color de relleno en el renderer
        if color is not None:
            self.glColor(*color)

        # 2) Scanline como antes
...
poly5 = [(682,175),(708,120),(735,148),(739,170)]  # agarre
...
def main():
    ...
    while running:
        ...
        # 2) Cuadrado y triángulo → blanco
        rend.glFillPolygon(poly2, WHITE)
        rend.glFillPolygon(poly3, WHITE)

        # 3) Tetera completa → blanco
        rend.glFillPolygon(poly4, WHITE)
        # 4) Hueco (agarre) → negro, recupera el fondo
